...
def test_user_create(api_client):
    """
    Testea la creación de un nuevo usuario llamando a /api/users/ [POST]
    payload = {
        "username": "",
        "email": "",
        "first_name": "",
    ...
    """
    ### Arrange Act Assert - si le pido pruebas a GPT pedirle que respete siempre Arrange ACt Assert
    ...
    assert user.check_password("fake123") is True

# @pytest.mark.django_db #!VER
...
def test_update_put_user(api_client): 
    ...
    payload = { #para el put le tengo que pasar todos los valores
    ...
    assert response.status_code == 200
    user.refresh_from_db() #actualiza el objeto user con la info de la db
...
def test_update_patch_user(api_client):
    ...
    payload = { #en el patch no necesito pasar todos los valores
        "first_name" : "Nombre Nuevo",
    }
...
def test_soft_delete_user(api_client):
    ...
    assert response.json()['detail'] == "Usuario desactivado correctamente."
